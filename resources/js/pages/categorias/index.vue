...
const onDelete = (id: number) => {
    if (window.confirm('¿Está seguro de eliminar esta categoría?')) {
        router.delete(categorias.destroy({ categoria: id }));
    }
};
</script>
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
        <div class="absolute top-1.5 right-4 flex items-center gap-2">
            <Button>
                <Link :href="categorias.create()" class="flex gap-1">
        ...
        <div class="mb-0">
            <Form method="GET" :href="categorias.index()" class="flex gap-2">
                <Input name="search" :default-value="search ?? ''" placeholder="Buscar categoría..." class="max-w-sm" />
                ...
                <TableRow v-for="item in collection.data" :key="item.id">
                    ...
                    <TableCell class="text-end">
                        <Button variant="secondary" size="sm" class="mr-2" title="Editar">
                            <Link :href="categorias.edit(item.id)"><Pencil /></Link>
                        </Button>
